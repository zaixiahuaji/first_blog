...
const handleSearchInput = () => {
  ...
  debounceTimer = window.setTimeout(() => {
    postsStore.setSearchQuery(searchInput.value)
  }, 500)
}
...
<template>
  <header
    class="h-20 border-b-2 border-[#2d2d30] bg-white flex items-center justify-between px-6 z-10 shrink-0"
  >
    <div class="flex items-center gap-4">
      ...
      <div class="hidden md:block">
        <h1
          ref="titleRef"
          class="text-3xl tracking-[0.2em] font-bold text-[#2d2d30] uppercase"
        >
          档案馆_84
        </h1>
    ...
    <div class="flex-1 max-w-md mx-4 relative group">
      <div
        class="flex items-center border-2 border-[#2d2d30] bg-[#f4f4f6] focus-within:bg-white transition-colors"
      ...
      >
        ...
        <button
          v-if="searchInput"
          @click="clearSearch"
          class="px-2 text-[#2d2d30] hover:text-[#e62e2e] font-bold"
        >
          X
        </button>
      </div>
    ...
    <div class="hidden md:flex gap-6 items-center">
      <div class="text-xs border border-[#e62e2e] text-[#e62e2e] px-2 py-1 uppercase bg-[#e62e2e]/5">
        录制 ●
      </div>
